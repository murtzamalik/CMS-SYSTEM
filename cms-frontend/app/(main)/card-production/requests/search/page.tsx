...
const requestTypeOptions = [
  ...
  { label: 'Change card type', value: 'CHANGE_TYPE' },
...
export default function CardRequestSearchPage() {
  ...
  const [filters, setFilters] = useState({
    ...
    requestTypeId: undefined as string | undefined,
    page: 0,
    size: 20,
  });
  ...
  const sourceBody = (r: CardRequest) => {
    ...
    const severity: 'info' | 'warning' | 'success' | undefined =
      label === 'Change card type' ? 'info' : label === 'Replacement' ? 'warning' : undefined;
    ...
    <div className="card">
      ...
      <div className="mb-4">
        ...
        <p className="m-0 text-color-secondary" style={{ fontSize: '0.875rem' }}>
          Find card requests by relationship, branch, request source, or processed status.
        </p>
      </div>
      <FormSection title="Filters" className="pb-3">
        <div className="flex flex-wrap align-items-end gap-2">
          ...
          <FormField label="Request source" htmlFor="search-request-type" className="mb-0">
            <Dropdown
              inputId="search-request-type"
              placeholder="All"
              value={filters.requestTypeId}
              options={requestTypeOptions}
              onChange={(e) => setFilters((s) => ({ ...s, requestTypeId: e.value, page: 0 }))}
              className="w-14rem"
            />
          </FormField>
